...
function UpdateSpot() {
	...
	const validateImages = () => spotDetails.images.every(
	imgUrl => /\.(jpeg|jpg|png)$/i.test(imgUrl)
	);

	const handleSubmit = async (e) => {
	e.preventDefault();
	if (!validateImages()) {
		setErrors({ ...errors, images: "All images must be in JPG, JPEG, or PNG format." });
		return;
	}
	try {
		await dispatch(updateExistingSpot(spotId, spotDetails));
	...
	<form className="UpdateSpotForm" onSubmit={handleSubmit}>
		...
		{/* Dynamic form generation for each field */}
		{Object.keys(spotDetails).map((key) => (
		key !== 'images' && (
			<label key={key}>
		...
		{spotDetails.images.map((img, index) => (
		<label key={index}>
			Image {index + 1}
			<input
			type="text"
		...
		<button type="submit" disabled={!validateImages()}>
		Update Spot
		</button>
	</form>
	);
