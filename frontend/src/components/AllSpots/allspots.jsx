...
function AllSpots() {
    ...
    const spots = useSelector((state) => {
        if (!state.spots || !Array.isArray(state.spots.list)) {
            console.error('Spots list is not an array or state.spots is undefined:', state.spots);
            return [];
        }
        return state.spots.list.map((spotId) => state.spots[spotId]).filter(spot => spot);
    });

    console.log('Final spots:', spots);

    if (spots.length === 0) {
        return <div>Loading spots...</div>; // Consider a possible loading indicator or a message
    }
